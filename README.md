...
- **Basic auth + sitemap-started crawls** — crawl password-protected staging / UAT environments by supplying HTTP basic-auth credentials, and start a crawl from a sitemap URL instead of a website URL (XML, sitemap index, `.xml.gz`, or a plain-text list). Both sit at the top of the crawler sidebar. Also fixes sitemap analysis, which was erroring out on every run.
...
- **HTTP basic auth** — tick *Site requires basic auth* and enter a username / password to crawl staging, pre-launch and UAT environments sitting behind a server-level auth prompt. Credentials are applied to every fetch for that site — pages, robots.txt, sitemaps, the Playwright renderer and the bot-challenge browser — and are scoped to the crawled host, so they are never sent to a third-party host the crawl touches. They live in the browser tab only: not written to disk, not stored in saved crawls. Without credentials, a 401 start page now says exactly that instead of reporting a one-page site.
- **Start from a sitemap** — the start-source switch at the top of the sidebar takes either a **website URL** (crawl by following links, the default) or a **sitemap URL**. Point it at `sitemap.xml`, a sitemap index (child sitemaps are walked for you), `.xml.gz`, or a plain-text URL list, and every page it lists is queued up front — the site to crawl is taken from the URLs inside, so no website URL is needed. Link discovery is unchanged on top of that, so pages that are weakly linked, behind a JS-built nav, or only reachable from a page the crawler can't get to still make it into the audit. Off-site and non-page URLs are dropped, and the same sitemap is reused for the post-crawl sitemap diff. Sitemaps behind basic auth load with the credentials from the field directly below.
